...
if continue_training:
    ...
    lstm_autoencoder = load_model('logs/' + folder_name + '/model.h5')
else:
    timesteps = 100
    batch_num = 32
    lstm_size = 256
    ...
    lstm_autoencoder = LSTM_AE(timesteps, lstm_size, latent_dim).get_model(lr)
...
result_plotter = AEResultPlotter(dataset=balances, plots_folder=plots_folder, model=lstm_autoencoder)

model_filepath = 'logs/' + folder_name + '/model.h5'
config_filepath = 'logs/' + folder_name + '/config.json'
model_saver = ModelSaver(model=lstm_autoencoder, model_filepath=model_filepath, config_filepath=config_filepath)

lstm_autoencoder.fit(balances, balances, batch_size=batch_num,
