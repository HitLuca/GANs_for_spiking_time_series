...
class WGAN_GP_AE:
    ...
    def _build_models(self):
        self._encoder = wgan_gp_ae_utils.build_encoder(self._latent_dim, self._timesteps)
        self._decoder_generator = wgan_gp_ae_utils.build_decoder(self._latent_dim, self._timesteps)
        self._critic = wgan_gp_ae_utils.build_critic(self._timesteps)

        self._ae_model, self._generator = wgan_gp_ae_utils.build_ae_model(self._encoder,
        ...
        self._critic_model = wgan_gp_ae_utils.build_critic_model(self._encoder, self._decoder_generator, self._critic,
                                                                  self._latent_dim,
    ...
    def train(self, dataset):
        ...
        neg_ones = -ones
        placeholder = np.zeros((self._batch_size, 1))
        ...
        while self._epoch < self._epochs:
            ...
            for _ in range(self._n_critic):
                ...
                critic_losses.append(self._critic_model.train_on_batch(inputs, [ones, neg_ones, placeholder])[0])
            ...
            for _ in range(self._n_generator_vae):
                ...
                losses = self._ae_model.train_on_batch(inputs, [ones, placeholder])
                generator_losses.append(losses[1])
                ae_losses.append(losses[2])
            ...
            self._losses[2].append(ae_loss)

            print("%d [C loss: %+.6f] [G loss: %+.6f] [AE loss: %+.6f]" % (self._epoch, critic_loss, generator_loss, ae_loss))
    ...
    def _save_losses(self):
        utils.save_losses_wgan_gp_ae(self._losses, self._img_dir + '/losses.png')
    ...
    def _save_models(self):
        ...
        os.mkdir(dir)
        self._ae_model.save(dir + 'ae_model.h5')
    ...
    def get_models(self):
        return self._generator, self._critic, self._ae_model, self._critic_model

    def _apply_lr_decay(self):
        lr_tensor = self._ae_model.optimizer.lr
